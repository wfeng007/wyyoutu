<%@page import="wyyoutu.dao.RsAlbumDao"%>
...
<%@ page import="wyyoutu.dao.RsAlbumDao" %>
<%@ page import="wyyoutu.model.RsAlbum" %>
<%!
//
%>
...
//
//获取展板列表数据
RsAlbumDao rsAlbumDao=(RsAlbumDao)SpringContextHolder.getApplicationContext().getBean("rsAlbumDao");

Map<String,Object> pm=new HashMap<String,Object>();
pm.put("ownerId",accountInfo.getUserId()); //当前登录的session。TODO其实可以考虑使用该http-request-query查看特定人员的album。
List<RsAlbum> albumList=rsAlbumDao.query(pm);

//
%>
<jsp:include page="/header.jsp" flush="true">
	<jsp:param name="header_title" value="album" />   
</jsp:include>
...
$(function() {	
	...
	*/
	...
	if(typeof(accountInfo)==='object' && typeof(accountInfo.userId)==="string"){	
		$("a#addAlbumBtn").click(function(event){
			...
			*/
			...
			//TODO 设置按钮不可继续点击 或替换为进度环。不可按下。
			$.post("./rjs/addAlbum", {action:"add",owner:accountInfo.userId, albumName: albumName }, //默认使用post,json对象其实会转变为form格式数据向后传
				function (data, textStatus){ //回调
					//data 可以是 xmlDoc, jsonObj, html, text, 等等.
					//this; 这个Ajax请求的选项配置信息，请参考jQuery.get()说到的this
					//data 当前为json对象
		...
		$("a.removeAlbumBtn").click(function(event){
			...
			//alert("a");
			$.post("./rjs/removeAlbum", {action:"remove",seqId:seqId }, //默认使用post,json对象其实会转变为form格式数据向后传//默认如果不指定发送的contenttype就会使用form方式。
	...
	}//if(typeof(accountInfo)==='object' && typeof(accountInfo.userId)==="string") end
	
});
</script>

<div id="container" class="container" >
	<div class="box col25 album"><h4>新增test</h4><hr><a role="button" data-toggle="modal" href="#myModal"><img alt="add" src="./res/image/folder_add.png"/></a></div>
<%
for (RsAlbum rA : albumList){
%>
	<!-- 布局调整 -->
	<div class="box col25 album">
		<h4><%=rA.getName()%><a seqId="<%=rA.getSeqId()%>" href="javascript:void(0);" class="btn btn-small btn-danger removeAlbumBtn" >删除</a></h4>
		<hr><a id="album" href="./albumeditor.jsp"><img alt="board" src="./res/image/folder.png"/></a>
	</div>
<%
}
%>
...
<div id="myModal" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-header">
		...
		<h3 id="myModalLabel">新增图册</h3>
	</div>
	<div class="modal-body">
		<h4>图册名字：</h4>
		<p><input id="albumName" type="text"/><a class="tooltip-test" title="" href="#" data-original-title="Tooltip">提示</a>
		</p>
		<h4>描述：</h4>
		<p><input id="albumDesc" type="text"/><a class="tooltip-test" title="" href="#" data-original-title="Tooltip">提示</a></p>
		<hr></hr>
		<h4>
		    说明：
		</h4>
		<p>描述可选。开发中。。。</p>
		<a id="addAlbumBtn" class="btn" href="javascript:void(0);">新增</a>
	</div>
