...
/**
 * 临时的web servlet工具。后续修改放入framework。 本类也包含了跳转的数据规范。
 * 
 * @author wfeng007
...
public class WebResult {
	...
	// 参数
	public static final String HANDLE_OPTION_KEY_AUTO_DELAY = "HANDLE_OPTION_KEY_AUTO_DELAY";// 当该参数为null时，则直接跳转。//TODO
																								// <0时为手动跳转
																								// >0时为页面自动定时跳转。
	...
	/**
	 * 设置json格式的数据。
	 * TODO 解析json格式中的某些字段作为内部转发中的基本信息，其他部分可以作为附加信息来处理。
	 * 之后可以作为转发jsp继续forward的参数用。
	 * @param json
	 * @return
	 */
	...
	public void sendToTraffic() {
		try {
			this.request.getRequestDispatcher("/result.jsp").forward(
